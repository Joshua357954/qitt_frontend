...
const Timetable = ({ className }) => {
	...
	return ( 
		...
		<Layout screen='Timetable Screen' title="Timetable" $return="/admin/timetable" selection={<Selection/>}>
	        
	        
	        {<Link to="/admin/updateTimetable" className="text-black pl-10"> Update Timetable </Link>}
	    		...
	    		<div className="w-full h-full flex flex-col items-center justify-center  ">
	    			...
	    			<div className="w-full bg-gray-200 sm:w-[100%] h-full bg-blac pl-8 flex flex-col items-center bg-yellow-10 pt-5">
	    				...
	    				<div className="w-full px-2 h-[7%] mb-7 sm:h-10 overflow-x-auto flex sm:justify-start gap-x-2">
	    					{
	    						days.map((item,idx) => {
									return <div onClick={() => setDay(item)} className={`capitalize flex text-black select-none justify-center px-2 font-bold text-sm items-center  ${item === day ? 'bg-blue-200 border-[1px] border-black' : "bg-gray-50"} rounded-xl`}> {item} </div>
	    						})
